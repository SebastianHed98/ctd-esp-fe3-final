...
const Detail = () => {
  ...
  if (loading)
    return (
      <div className={`page-${state.theme} detail-loading`}>Cargando...</div>
    );
  if (error)
    return (
      <div className={`page-${state.theme} detail-error`}>Error: {error}</div>
    );
  if (!dentist)
    return (
      <div className={`page-${state.theme} detail-not-found`}>
        No se encontró el dentista
      </div>
    );
